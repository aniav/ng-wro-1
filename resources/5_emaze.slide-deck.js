...
EM_slideDeck = (function () {
    ...
    function getSlideDeckFromDocument() {
        var $dataContainer = $('#slidedeck > xmp'),
            ...
            slideDeckString, isJson, slideDeck,
            EM = EM || {};
        if (hasData) {
    ...
    //function decodeSlidesInDeck(slideDeck) {
    //    var section;
    //    try {
    //        for (var i = 0; i < slideDeck.sections.length; i++) {
    //            section = slideDeck.sections[i];
    //            for (var j = 0; j < section.slides.length; j++) {
    //              //  section.slides[j] = section.slides[j]; // decodeHtml(section.slides[j]); 
    //                section.slides[j] =  decodeURI(section.slides[j]); // decodeHtml(section.slides[j]); 
    ...
    //        console.error(e);
    //        if (EM_Editor) {
    //            EM_Editor.reportError('emaze.slide-deck.js/decodeSlidesInDeck', e);
    //        }
    //    }
    //}
    ////TODO rename to uri not base64
    //function getSlideDeckfromUncompressedBase64slides(slideDeckString) {
    ...
    //            decodeSlidesInDeck(slideDeck);
    //            return slideDeck;
    //        } else { //support old slide decks wiout base 64 encode and without json
    ...
    //function buildSlideDeckFromXML($slidesHtml) {
    //    var slideDeck = { sections: [], slideSettings: [] }, $settingsHtml, $allSlides, $addedSlides = $(), $misplacedSlides, lastsection;
    //    try {
    ...
    //            var $this = $(this),
    //                title = $this.data('title'),
    //                $slideIsolators = $this.children('code'), // using xmp to prevent parsing of slide html and thereby islotate errors.
    //                slides = [];
    //            $slideIsolators.each(function () {
    //                var $isolator = $(this);
    //                slides.push($isolator.html());
    //                slideDeck.slideSettings.push($isolator.data()); 
    //            });
    //            slideDeck.sections.push({ title: title, slides: slides });
    //        });
    //        //uncomment these lines once we implement presentation settings
    //       // $settingsHtml = $slidesHtml.children('settings');
    //      //  $settingsHtml.appendTo('#slide-container'); //only for editor. 
    //      //  slideDeck.PresentationSettings = settingsObjFromHtml($settingsHtml);
    //        return slideDeck;
